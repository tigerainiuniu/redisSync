    ...
    def _migrate_with_scan(self,
        ...
        while True:
            ...
            try:
                ...
                scanned_batch_len = len(keys) if keys else 0
                ...
                if keys:
                    ...
                    if getattr(self, "_key_filter", None) and keys:
                        keys = self._key_filter.filter_batch(self.source_client, list(keys))
                ...
                logger.error(
                    "SCAN迁移迭代失败: cursor=%s scan_count=%s scanned_batch_len=%s: %s",
                    cursor,
                    scan_count,
                    scanned_batch_len,
                    e,
            ...
            try:
                ...
                if isinstance(response, bytes):
                    ...
                    return {
                        ...
                        'migrated_keys': self.migration_stats.get('migrated_keys', 0),
            ...
            try:
                ...
                if keys:
                    ...
                    if key_types:
                        ...
                        for key in keys:
                            try:
                                key_type = _type_str(self.source_client.type(key))
                ...
                logger.error(
                    "DUMP/RESTORE SCAN迭代失败: cursor=%s scan_count=%s scanned_batch_len=%s: %s",
                    cursor,
                    scan_count,
                    scanned_batch_len,
                    e,
