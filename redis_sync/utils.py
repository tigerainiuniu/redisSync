...
from .exceptions import RedisTimeoutError
...
def timeout_context(timeout_seconds: float, operation_name: str = "Operation"):
    ...
    if not hasattr(signal, "SIGALRM"):
        logger.warning(
            "当前平台不支持 SIGALRM，timeout_context 将不生效: %s",
            operation_name,
        )
        yield
        return
...
def validate_redis_key(key: str) -> bool:
    """
    校验键是否适合作为「文本日志/配置」场景的键名。

    Redis 服务端允许二进制键（含 \\x00）；此处仅做长度与非空检查。
    """
    if not key:
        return False

    if len(key) > 512 * 1024 * 1024:
        return False
